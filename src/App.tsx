...
const App: FC<AppProps> = () => {
  ...
  useEffect(() => {
    ...
    const newMessageHandler = (newMessage: MessageType, chat: ChatType) => {
      if (chat && chat._id === selectedChat._id) {
        ...
        if (notification_settings.messages_notifications) {
          sendNotification("New Message!", `${newMessage.sender.name}: ${newMessage.msgType !== "text" ? newMessage.fileName : newMessage.message}`, `new message id: ${newMessage._id}`, `http://localhost:5173/chat/${chat._id}`)
        }
    ...
    const reactOnMessages = (
      ...
      if (chat && chat._id === selectedChat._id) {
        ...
        if (message.sender._id === loggedInUser._id && user._id !== loggedInUser._id && notification_settings.react_on_message_notifications) {
          sendNotification("Reacted on message", `${user.name} reacted "${emoji}" on your message`, `reaction ${message._id}`, `http://localhost:5173/chat/${chat._id}`)
        }
      }
    };
    ...
    const promoteAdminFunc = (
      ...
      console.log("promote admin is now working fine");
      if (chat && chat._id === selectedChat._id) {
    ...
    const messageSeenFunc = (messageIds: string[], chat: ChatType, user: UserType) => {
      if (chat && chat._id === selectedChat._id) {
        console.log("messagessIdLenght", messageIds)
